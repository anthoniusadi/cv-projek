...
def save():
    ...
    format_file.append('Gender : '+gender_entry.get())
...
gender = Label(master,text='Gender (L/P)').grid(row=1)
umur = Label(master, text='Umur').grid(row=2)
rekam_medis = Label(master, text='No.Rekam Medis').grid(row=3)
nama_dokter = Label(master, text='Nama Dokter').grid(row=4)
nama_rs = Label(master, text='Nama Rumah Sakit').grid(row=5)
...
rekam_medis_entry.grid(row=3, column=1)
nama_dokter_entry.grid(row=4, column=1)
nama_rs_entry.grid(row=5, column=1)
